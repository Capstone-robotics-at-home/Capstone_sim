...
from gym_rev import error, logger
...
class EnvRegistry(object):
    ...
    def make(self, path, **kwargs):
        ...
        # reset/step. Set _gym_rev_disable_underscore_compat = True on
        # your environment if you use these methods and don't want
        # compatibility code to be invoked.
        if hasattr(env, "_reset") and hasattr(env, "_step") and not getattr(env, "_gym_rev_disable_underscore_compat", False):
            patch_deprecated_methods(env)
        if env.spec.max_episode_steps is not None:
            from gym_rev.wrappers.time_limit import TimeLimit
    ...
    def spec(self, path):
        if ':' in path:
            ...
            # catch ImportError for python2.7 compatibility
                ...
                raise error.Error('A module ({}) was specified for the environment but was not found, make sure the package is installed with `pip install` before calling `gym_rev.make()`'.format(mod_name))
        else:
            id = path
...
def patch_deprecated_methods(env):
    ...
    global warn_once
    if warn_once:
        logger.warn("Environment '%s' has deprecated methods '_step' and '_reset' rather than 'step' and 'reset'. Compatibility code invoked. Set _gym_rev_disable_underscore_compat = True to disable this behavior." % str(type(env)))
        warn_once = False
